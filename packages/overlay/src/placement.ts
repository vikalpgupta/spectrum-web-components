...
export const updateOverlayPosition = async (
    ...
    const mainAxis = Array.isArray(options?.offset) ? options?.offset[0] : options?.offset || 0;
    const crossAxis = Array.isArray(options?.offset) ? options?.offset[1] || 0 : 0;
    ...
    if (overlayData.tipElement && middlewareData.arrow) {
        ...
        Object.assign(overlayData.tipElement.style, {
            top: '0px',
            left: '0px',
            transform: `translate(${roundByDPR(arrowX)}px, ${roundByDPR(arrowY)}px)`,
        });
    }
};
