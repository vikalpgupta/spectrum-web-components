...
function getContainingBlock(element: Element) {
    ...
    while (isHTMLElement(currentNode) && !isLastTraversableNode(currentNode)) {
        if (isContainingBlock(currentNode)) {
            ...
            const parent = (currentNode.assignedSlot ? currentNode.assignedSlot : currentNode.parentNode) as Node;
...
export function isShadowRoot(node: Node): node is ShadowRoot {
    ...
    const OwnElement = (getWindow(node) as unknown as { ShadowRoot: ShadowRoot }).ShadowRoot;
    const testNode = node as Node;
    return node instanceof (OwnElement as unknown as any) || testNode instanceof ShadowRoot;
}

export function isHTMLElement(value: any): value is HTMLElement {
    return value instanceof (getWindow(value) as unknown as { HTMLElement: any }).HTMLElement;
}
