...
import { executeServerCommand, findAccessibilityNode } from '@web/test-runner-commands';
...
describe('Combobox', () => {
    ...
    describe('renders accessibly', () => {
        ...
        it('manages aria-activedescendant', async () => {
            ...
            expect(
                findAccessibilityNode<ActiveDescendentFindableNode>(
                    snapshot,
                    (node: ActiveDescendentFindableNode) => !!node.activedescendent
                )
            ).to.be.null;
            snapshot = (await executeServerCommand(
                'a11y-snapshot'
            ...
            const activeDescendent = findAccessibilityNode(
                snapshot,
                (node: ActiveDescendentFindableNode) => node.role === 'combobox'
            );
        ...
        xit('manages aria-selected', async () => {
            ...
            let snapshot = (await executeServerCommand(
                'a11y-snapshot'
            )) as SelectedNode & { children: SelectedNode[] };
        ...
        it('manages aria-expanded', async () => {
            ...
            let snapshot = (await executeServerCommand(
                'a11y-snapshot'
            )) as ExpandedNode & { children: ExpandedNode[] };

            expect(
                findAccessibilityNode<ExpandedNode>(
                    snapshot,
                    (node: ExpandedNode) => !!node.expanded
                )
            ).to.be.null;
