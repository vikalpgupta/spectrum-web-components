...
export class Combobox extends SpectrumElement {
    ...
    public onListPointerenter({
        target,
    }: PointerEvent & { target: ComboboxItem }): void {
        this.activeDescendent = target;
    }

    public onListPointerleave(): void {
        this.activeDescendent = undefined;
    }

    public async onListClick(event: PointerEvent & { target: ComboboxItem }): Promise<void> {
        const { target } = event;
        event.preventDefault();
        this.activeDescendent = target;
        if (!this.activeDescendent) {
        ...
        this.selectDescendent();
        this.open = false;
        if (!this._returnItems) return;
        (await this._returnItems)();
        delete this._returnItems;
    }
    ...
    private _returnItems?: Promise<() => void>;

    protected async manageListOverlay(): Promise<void> {
        if (this.open) {
            this._returnItems = openOverlay(
